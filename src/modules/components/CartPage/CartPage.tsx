...
import { useAppDispatch, useAppSelector } from '../../shared/hooks/redyxTypes';
import { isFilled } from '../../../features/cart';

export const CartPage: React.FC = () => {
  const { btnsTitle, notifMessage, navList, additionalText } =
    useContext(TranslationContext);
  const [modal, setModal] = useState(false);
  const { cartList, cartFilled } = useAppSelector(state => state.cart);
  ...
  useEffect(() => {
    if (cartList.length > 0) {
      dispatch(isFilled(true));
      setTimeout(() => {
        dispatch(isFilled(false));
      }, 2000);
    }
  }, []);
